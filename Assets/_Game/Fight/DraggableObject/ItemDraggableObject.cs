...
public class ItemDraggableObject : DraggableObject
{
    // --- 1. 事件註冊 (只在子類別處理) ---
    ...
    {
        Debug.Log("收到 Boss Idle 事件：解除鎖定，玩家可調整ItemButton位置。");
    }
    ...
    {
        Debug.Log("玩家原地確認,物品欄開啟");
        TriggerItemWindowOpen();
    }
    ...
    private void TriggerItemWindowOpen()
    {
        // 1. 發送物品欄開啟事件
        Debug.Log("位置確認！鎖定拖曳，發送物品欄開啟訊號。");
        GameManager.Instance.MainGameEvent.Send(new ItemButtonClickEvent());
    }
}
